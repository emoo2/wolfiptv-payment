jQuery((function($) {
    ...
    "function" == typeof $.fn.slick && ($(".pwb-carousel").slick({
    ...
    var PWBFilterByBrand = function() {
        ...
        $('.pwb-filter-products input[type="checkbox"]').each((function(index) {
            $(this).prop("checked") && marcas.push($(this).val())
        }
        )),
        currentUrl = (marcas = marcas.join()) ? -1 === (currentUrl = (currentUrl = currentUrl.replace(/&?pwb-brand-filter=([^&]$|[^&]*)/i, "")).replace(/\/page\/\d*\//i, "")).indexOf("?") ? currentUrl + "?pwb-brand-filter=" + marcas : currentUrl + "&pwb-brand-filter=" + marcas : baseUrl,
        location.href = currentUrl
    }
      , PWBRemoveFilterByBrand = function() {
        var baseUrl = [location.protocol, "//", location.host, location.pathname].join("")
          , currentUrl = window.location.href;
        currentUrl = (currentUrl = currentUrl.replace(/&?pwb-brand-filter=([^&]$|[^&]*)/i, "")).replace(/\/page\/\d*\//i, ""),
        location.href = currentUrl
    };
    ...
    if (null != brands) {
        ...
        for (var i = 0, l = brands_array.length; i < l; i++)
            $('.pwb-filter-products input[type="checkbox"]').each((function(index) {
                $(this).val() && brands_array[i] == $(this).val() && $(this).prop("checked", !0)
            }
            ))
    } else
        $('.pwb-filter-products input[type="checkbox"]').prop("checked", !1)
...
var PWBgetUrlParameter = function PWBgetUrlParameter(sParam) {
    var sPageURL, sURLVariables = decodeURIComponent(window.location.search.substring(1)).split("&"), sParameterName, i;
    for (i = 0; i < sURLVariables.length; i++)
        if ((sParameterName = sURLVariables[i].split("="))[0] === sParam)

...
function initEvents() {
    ...
    jQuery('.swipebox.image-video, .swipebox.ltx-video-popup').magnificPopup({
        type: 'iframe'
    });
    ...
    var searchHandler = function(event) {
        ...
        if (jQuery(event.target).is(".top-search, .top-search *"))
            return;
    ...
    jQuery(ltxSliders).each(function(i, el) {
        ...
        jQuery(breakpoints_per).each(function(i, el) {
            ...
            if (!slidesPerView && el) {
                ...
                slidesPerView = el;
                if (container.data('slides-per-group'))
                    slidesPerGroup = el;
                slidesPerGroup = 1;
            }
            ...
            if (el) {
                ...
                if (spg == 1)
                    delete breakpoints[breakpoints_viewports[i]]['slidesPerGroup'];
            }
        });
...
function initSwiper() {
    ...
    if (gallerySwiperEl.length) {
        ...
        var gallerySlides = 7;
        if (gallerySwiperEl.hasClass('grid-big'))
            gallerySlides = 4;
